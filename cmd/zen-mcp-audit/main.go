...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "zen-mcp-audit: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	fs := flag.NewFlagSet("zen-mcp-audit", flag.ContinueOnError)
	socket := fs.String("socket", "/var/run/zen-swarm/zen-swarm.sock", "daemon Unix socket path")
	...
	minPoolSize := fs.Int("min-pool-size", 2, "minimum disjoint reviewer pool size (inv-zen-080)")
