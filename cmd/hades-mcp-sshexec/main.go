...
// Usage (by plugin):
//
// hades-mcp-sshexec --doctrine default --project-id hades-system
//
// Usage (operator debug with custom doctrine file):
//
// hades-mcp-sshexec --doctrine-file /path/to/doctrine.toml --project-id hades-system
//
// # Flags
//
...
// --project-id per-project doctrine identifier (default: "hades-system")
// --project-toml path to per-project hadessystem.toml (default: "")
package main
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "hades-mcp-sshexec: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	fs := flag.NewFlagSet("hades-mcp-sshexec", flag.ContinueOnError)
	...
	projectID := fs.String("project-id", "hades-system", "per-project doctrine identifier")
