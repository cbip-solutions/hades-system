...
func (w *CronWorker) runWithInterval(ctx context.Context, heartbeatInterval time.Duration) error {
	...
	w.logger.Info("zen-docs-cron started",
	...
	for {
		select {
		case <-ctx.Done():
			w.logger.Info("zen-docs-cron stopping", slog.String("reason", ctx.Err().Error()))
			return nil
...
//
// Inv-zen-204: sweep is idempotent — re-running produces zero schema diff.
...
func parseFlags(fs *flag.FlagSet, args []string) (runtimeConfig, error) {
	...
	fs.StringVar(&cfg.daemonSocket, "daemon-uds", "/tmp/zen-swarm.sock", "daemon Unix socket path")
...
func buildCronWorker(
	...
	if err != nil {
		return nil, fmt.Errorf("zen-docs-cron: %w", err)
	}
	pollInterval, err := time.ParseDuration(cfg.pollInterval)
	if err != nil {
		return nil, fmt.Errorf("zen-docs-cron: invalid poll-interval %q: %w", cfg.pollInterval, err)
	}
