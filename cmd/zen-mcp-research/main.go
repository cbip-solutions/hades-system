...
// cmd/zen-mcp-research/main.go is the thin entrypoint for the research
...
func main() {
	if err := run(); err != nil {
		log.Fatalf("zen-mcp-research: %v", err)
	}
}

func run() error {
	fs := flag.NewFlagSet("zen-mcp-research", flag.ContinueOnError)
