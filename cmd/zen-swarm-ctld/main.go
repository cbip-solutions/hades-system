...
func main() {
	udsPath := flag.String("uds", "/tmp/zen-swarm.sock", "Unix domain socket path")
	httpAddr := flag.String("http", "", "Optional TCP HTTP address (e.g. 127.0.0.1:8443)")
	dbPath := flag.String("db", "", "SQLite path (default ~/.local/share/zen-swarm/state.db)")
	...
	if versionFlag != nil && *versionFlag {
		...
		fmt.Printf("HADES system v%s (binary: zen-swarm-ctld)\n%s\n", brandVersion, buildinfo.Summary())
		os.Exit(0)
	}
	...
	if err := bootDoctrineRegistry(); err != nil {
		logger.Error("doctrine registry wire-up (inv-zen-134)", "err", err)
		os.Exit(1)
	}
	logger.Info("Plan 11 doctrine registry live",
		"builtins", []string{"max-scope", "default", "capa-firewall"},
		"effect", "active.Accessor singleton populated; /v1/doctrine/active + zen://audit "+
	...
	providersConfigDir := filepath.Join(os.Getenv("HOME"), ".config", "zen-swarm", "providers")
	...
	if _, probeErr := profileResolver.Resolve("", ""); probeErr != nil {
		logger.Warn("daemon: no default routing profile configured",
			"effect", "unlabeled POST /v1/messages (e.g. a Hermes chat with no X-Zen-Profile) is rejected",
			"fix", "set `default = \"<profile>\"` (e.g. \"orchestrator\") in "+filepath.Join(providersConfigDir, "routing.toml")+", or send an X-Zen-Profile header")
	}
	...
	if cerr := verifyCascadeCompleteness(profileResolver, providerRegistry); cerr != nil {
		logger.Error("daemon: inv-zen-211 cascade-completeness check failed", "err", cerr)
		os.Exit(1)
	}
	...
	if err := built.CostCounters.RebuildFromLedger(time.Now().Add(-30 * 24 * time.Hour)); err != nil {
		logger.Error("cost counters rebuild from ledger (inv-zen-065)",
			"err", err,
	...
	if caronteErr != nil {
		logger.Error("mcpgateway: caronte engine bootstrap (bootstrap-required, generalises inv-zen-206)",
			"err", caronteErr,
	...
	if augmentHandler == nil {
		logger.Warn("Plan 11 augmentation: NOT WIRED",
			...
			"resolution", "operator runs `zen knowledge init` to materialize the Plan 9 D vault; daemon restart picks up the wired substrate")
	} else {
	...
	configDir := filepath.Join(os.Getenv("HOME"), ".config", "zen-swarm")
	...
	logger.Warn("Plan 7 day generator: NOT WIRED",
		"routes", "/v1/zen-day/{morning,eod,check-pending}",
		"effect", "503 day_unavailable until Plan 8 composes zenday.GeneratorDeps "+
	...
	//
	// stateDir resolves to $XDG_STATE_HOME/zen-swarm or
	// ~/.local/state/zen-swarm. The litestream-configs/ subdirectory is
	...
	ecoConfigDir := filepath.Join(os.Getenv("HOME"), ".config", "zen-swarm", "providers")
	...
	logger.Info("HADES system daemon (zen-swarm-ctld) starting",
		"version", Version, "uds", *udsPath, "http", *httpAddr, "db", *dbPath)

	go func() {
		if err := RegisterZenScheme(ctx); err != nil {
			if errors.Is(err, ErrUnsupportedPlatform) {
				logger.Warn("zen:// URL scheme registration unsupported on this platform",
					"manual_fallback", "zen audit event <id>",
					"err", err)
			} else {
				logger.Warn("zen:// URL scheme registration failed (non-fatal)",
					"err", err)
			}
		} else {
			logger.Info("zen:// URL scheme registered")
		}
	}()
	...
	logger.Info("HADES system daemon (zen-swarm-ctld) stopped")
}
...
func caronteJinaScriptPath() string {
	const relPath = "internal/research/ecosystem/scripts/zen_jina_embed.py"
