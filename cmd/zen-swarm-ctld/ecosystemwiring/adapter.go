...
// helper (TryWire) that cmd/zen-swarm-ctld main.go calls to inject it.
//
...
// at cmd/.
//
// Why a sub-package (not in package main): the cmd/zen-swarm-ctld test
...
var ErrEcosystemDBNotConfigured = errors.New(
	"ecosystem: per-ecosystem DB not configured for ecosystem (run `zen providers init` then restart daemon)",
)
