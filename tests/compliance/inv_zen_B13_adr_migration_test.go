// Package compliance — Task B-13 ADR migration sister-test.
//
// policy (ADRs public by default, with a 4-bypass exception list),
...
// authority policy (the BYPASS POLICY BLOCK).
//
// Gate shape (defense-in-depth, two anchors):
//
// 1. **Absence in the public dev repo** — none of the four bypass ADR
// filenames may exist under `architecture records` in this repo. The
...
// `B13` is the task ID, NOT the final inv-zen number).
//
...
// that ships ). Both gates remain orthogonal: B-6 partitions
...
// forbiddenBypassADRs is the canonical policy exception list of
...
// authored at documenting policy as the canonical
...
// IDs for historical traceability policy
...
// ADRs are ABSENT from this repo's `architecture records` policy
//
