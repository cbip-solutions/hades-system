...
// Package keychain is the shared OS-credential resolver for zen-swarm.
//
...
// internal/providers may not import (inv-zen-031). This package is
// boundary-neutral — importing it from internal/providers does NOT
// violate inv-zen-031 (it imports only internal/redact + stdlib +
// github.com/keybase/go-keychain on darwin).
//
// Convention service = "zen-swarm/<provider>", account = "zen-swarm" —
...
// blocker — ZEN_BYPASS_DISABLE_KEYCHAIN=1 precedent). When
// ZEN_KEYCHAIN_DISABLE=1 is set, SystemResolver.Lookup reads the
// credential from a ZEN_KEYCHAIN_<SERVICE> environment variable instead
...
// validateServiceName enforces the convention service = "zen-swarm/<provider>"
...
//
// inv-zen-068 (no silent credential mismatch): a well-formed service maps
...
const disableEnvVar = "ZEN_KEYCHAIN_DISABLE"
