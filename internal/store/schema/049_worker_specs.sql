--
-- Migration 049: worker specs persistence (Plan 4 Phase D Task D-8).
...
-- explicit and let Plan 5 + Plan 9 add variant-specific columns
-- without conditional checks at the call site.
--
-- All three carry project_id (spec §7.1 logical isolation; Phase B
-- queues already enforce this on every row).
--
-- inv-zen-031 boundary preserved: internal/workforce/worker MUST NOT
-- import internal/store. The daemon adapter (Phase G) will wire the
-- read/write surface; Phase D ships only the schema so Plan 5
