-- ==============================================================================
-- Migration 061: Knowledge index extension hooks (Plan 7 Phase G; spec §1 Q17 D)
-- ==============================================================================
--
...
-- §2.4 Plan 7 = migrations 057-061 reservation, §2.4 final entry).
-- The actual schema is materialized at runtime by internal/knowledge/index.go
-- Init function. This .sql file serves as:
--   (a) cross-Plan documentation (Plan 9 hash-chain + Plan 14 RAG +
--       gitnexus-mcp grep here for the canonical extension-hook column list);
--   (b) `make verify-invariants` grep target (compliance tests inv-zen-129
--       and inv-zen-130 reference this file by path);
...
-- by default in Plan 7. Plan 9 fills audit_chain_anchor at audit-event
-- materialization time (separate writer, separate boundary). Plan 14
-- fills ecosystem_join_keys at RAG-link discovery time. Caronte fills
-- caronte_symbol_refs at code-graph reverse-link discovery time (Plan 19).
-- Plan 7 INSERT statements NEVER populate any of these three columns;
-- inv-zen-130 compliance test (G-16) enforces.
--
...
-- Rationale (spec §1 Q17 D): FTS5 ALTER TABLE constraints would block Plan 9 /
-- Plan 14 / gitnexus column adds. Keeping FTS5 minimal + supplementary
...
CREATE TABLE IF NOT EXISTS knowledge_meta (
    ...
    -- Extension-hook columns: NULL by default in Plan 7 (inv-zen-130).
    audit_chain_anchor   TEXT,                           -- Plan 9 fills (hash-chain anchor)
    ecosystem_join_keys  TEXT,                           -- Plan 14 reads (URLs in content; JSON array)
    caronte_symbol_refs  TEXT                            -- Caronte reverse-links from markdown (JSON array; Plan 19)
);
