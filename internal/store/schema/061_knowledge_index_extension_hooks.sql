...
--   (a) cross-Plan documentation (HADES design hash-chain + HADES design RAG +
...
-- caronte_symbol_refs at code-graph reverse-link discovery time (HADES design).
...
CREATE TABLE IF NOT EXISTS knowledge_meta (
    ...
    -- Extension-hook columns: NULL by default in HADES design (invariant).
    audit_chain_anchor   TEXT,                           -- HADES design fills (hash-chain anchor)
    ecosystem_join_keys  TEXT,                           -- HADES design reads (URLs in content; JSON array)
    caronte_symbol_refs  TEXT                            -- Caronte reverse-links from markdown (JSON array; HADES design)
);
