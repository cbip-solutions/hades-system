...
-- recursion. inv-hades-143 enforced via REFUSE triggers.
--
-- Boundary (inv-hades-031): writes go through auditadapter (which imports
...
--   inv-hades-143: audit_events_raw append-only — REFUSE triggers enforce
...
-- ======================================================================
-- REFUSE triggers — append-only enforcement (inv-hades-143)
-- ======================================================================
--
...
BEGIN
    SELECT RAISE(FAIL, 'audit_events_raw is append-only (the release design chain integrity inv-hades-143); immutable columns id/project_id/type/payload_json/emitted_at cannot be modified');
END;
...
BEGIN
    SELECT RAISE(FAIL, 'audit_events_raw is append-only (the release design chain integrity inv-hades-143); chain hashes cannot be rewritten once computed');
END;
...
BEGIN
    SELECT RAISE(FAIL, 'audit_events_raw is append-only (the release design chain integrity inv-hades-143); partition_id cannot be rewritten');
END;
...
BEGIN
    SELECT RAISE(FAIL, 'audit_events_raw is append-only (the release design chain integrity inv-hades-143); tessera_leaf_id cannot be rewritten once batch sealed');
END;
...
BEGIN
    SELECT RAISE(FAIL, 'audit_events_raw is append-only (the release design chain integrity inv-hades-143); DELETE is forbidden');
END;
