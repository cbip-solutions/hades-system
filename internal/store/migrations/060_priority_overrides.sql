-- Migration 060: priority_overrides + tmux_session_state (HADES design stage+C joint).
--
-- Reservation: per master plan §"Migration numbering coordination", 060 is the
-- joint migration shared by stage (priority_overrides) and stage
-- (tmux_session_state). stage ships priority_overrides because the quota
-- override storage seam needs it now. stage will add tmux_session_state in a
-- subsequent migration (e.g., 060a or a new number) — stage leaving room
-- here would invite a half-empty migration; stage owns its own DDL.
--
...
--                   (stage) physically removes rows where expires_at < now.
--   - reason TEXT NOT NULL: audit trail demands operator intent (spec §1 design choice).
