...
var catalog = map[Code]*CatalogEntry{
	...
	"daemon.not-running": {
		...
		RecoveryHint: "run: hades daemon start (one-shot) OR brew services start cbip-solutions/tap/hades (Homebrew); macOS source installs may use: hades daemon install; Linux source installs can run bin/hades-ctld in the foreground or provide their own user service; see docs/operations/daemon.md",
	...
	"daemon.unreachable": {
		...
		RecoveryHint: "the socket may be stale: rm /tmp/hades-system.sock && hades daemon start; if Homebrew-managed: brew services restart cbip-solutions/tap/hades; see docs/operations/daemon.md",
		Severity:     SeverityError,
		Category:     CategoryDaemon,
	},
	...
	"daemon.endpoint-not-found": {
		...
		RecoveryHint: "verify CLI / daemon versions match: hades --version && hades doctor --component=daemon; if a recent upgrade moved the route (e.g. HADES design moved /v1/caronte/* to /v1/mcpgateway/*), reinstall both via the same release: brew reinstall hades or make build from a clean source checkout. Do NOT delete /tmp/hades-system.sock for endpoint-404; the daemon is reachable.",
		Severity:     SeverityError,
		Category:     CategoryDaemon,
	},
	...
	"daemon.version-mismatch": {
		...
		RecoveryHint: "verify: hades --version && hades doctor --component=daemon (will print daemon version); if mismatched, restart the daemon with brew services restart cbip-solutions/tap/hades or stop the foreground bin/hades-ctld process and restart it; reinstall both CLI and daemon from the same release with brew reinstall hades or make build from this repo",
		Severity:     SeverityError,
		Category:     CategoryDaemon,
	},
	...
	"daemon.auth-failed": {
		...
		RecoveryHint: "run: hades doctor --component=auth (prints token file location + validity); if invalid, run: rm ~/.config/hades-system/daemon.token && brew services restart cbip-solutions/tap/hades, or restart your foreground bin/hades-ctld process; see docs/operations/security.md §daemon-auth for the token-lifecycle reference",
		Severity:     SeverityError,
		Category:     CategoryDaemon,
	},
	...
	"wizard.config-corrupt": {
		...
		RecoveryHint: "inspect: cat ~/.config/hades-system/config.toml | head -50 (check for syntax errors); to start fresh, run: mv ~/.config/hades-system/config.toml ~/.config/hades-system/config.toml.bak && hades config init (re-launches the wizard with a clean slate); see CONFIGURATION.md and docs/operations/first-five-minutes.md for the expected first-run layout",
		Severity:     SeverityError,
	...
	"wizard.migrate-incomplete": {
		...
		RecoveryHint: "review skipped items: hades migrate HADES design --dry-run (lists what would still change); to retry, run: hades migrate HADES design --apply (idempotent — safe to invoke multiple times); see docs/operations/troubleshooting.md for first-run recovery guidance",
		Severity:     SeverityWarn,
		Category:     CategoryWizard,
	},
	...
	"wizard.mcp-spawn-fail": {
		...
		BodyTemplate: "The wizard tried to spawn one of the 4 HADES MCP servers (research, budget, audit, ssh-exec) to verify Hermes plugin integration; the spawn failed (binary not on PATH, permission denied, or transport handshake error).",
		RecoveryHint: "diagnose: hades doctor --component=mcp (probes each MCP server with verbose output); verify PATH: which hades-mcp-research && which hades-mcp-budget; if missing, run: make build (rebuilds all local binaries); see docs/integrations/hermes-and-mcp.md for MCP server-specific install steps",
		Severity:     SeverityError,
		Category:     CategoryWizard,
	},

	"wizard.preflight-fail": {
		Code:         "wizard.preflight-fail",
		Title:        "Onboarding preflight failed.",
		BodyTemplate: "HADES cannot enter the onboarding wizard until required local prerequisites are present. Common first-run causes are a missing or outdated Hermes Agent binary, missing bash on minimal Linux images, or a local preflight hook failure.",
		RecoveryHint: "install Hermes Agent first (Homebrew users get it through brew install hades; Linux/source users can use Homebrew/Linuxbrew or the Hermes Agent install path), verify with: hermes --version, then rerun: hades config init or hades init; see INSTALL.md and docs/operations/first-five-minutes.md",
		Severity:     SeverityError,
		Category:     CategoryWizard,
	},
	...
	"plugin.load-error": {
		...
		RecoveryHint: "inspect: hermes plugins list --verbose (shows per-plugin load status + last error); Homebrew reinstall/link: rm -rf ~/.hermes/plugins/hades && mkdir -p ~/.hermes/plugins && ln -sfn \"$(brew --prefix hades)/share/hades/hades\" ~/.hermes/plugins/hades; source checkout: make plugin-install; verify Python: python3 --version; see docs/integrations/hermes-and-mcp.md for the plugin activation flow",
		Severity:     SeverityError,
	...
	"plugin.mcp-handshake-fail": {
		...
		RecoveryHint: "diagnose: hades doctor --component=mcp (probes each MCP transport with verbose output); reinstall MCP manifests: hades plugin install-mcps; see docs/integrations/hermes-and-mcp.md for the handshake and daemon-socket contract",
		Severity:     SeverityError,
		Category:     CategoryPlugin,
	},
	...
	"cli.unknown-subcommand": {
		...
		RecoveryHint: "list valid subcommands: hades --help (top-level command tree); to find a specific subcommand by keyword: hades --help | grep -i <keyword>; see docs/reference/cli.md for the public CLI surface",
		Severity:     SeverityError,
	...
	"skin.skin-not-registered": {
		...
		RecoveryHint: "verify: ls -la ~/.hermes/skins/hades.toml (expected: regular file, readable by current user); reinstall the skin: hades --install-skin (re-deploys from binary); see docs/integrations/hermes-and-mcp.md for Hermes integration recovery",
		Severity:     SeverityError,
	...
	"migrate.symlink-out-of-scope": {
		...
		RecoveryHint: "inspect the symlink: readlink -f {{.rel}} (shows the resolved target); if the target should be migrated, follow the symlink manually and re-run the tool with --home pointing to the target's home dir; see docs/operations/troubleshooting.md for migration recovery guidance",
		Severity:     SeverityWarn,
		Category:     CategoryMigrate,
	},
