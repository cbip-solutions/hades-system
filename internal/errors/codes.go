...
var catalog = map[Code]*CatalogEntry{
	...
	"bypass.tier-degraded": {
		...
		RecoveryHint: "diagnose: hades doctor --component=bypass (shows recent error breakdown + token-refresh history); inspect last failures: hades bypass tail-errors --limit=20; if the token has expired, run: hades bypass extract-config (re-extracts from a fresh Claude Code session)",
		Severity:     SeverityWarn,
		Category:     CategoryBypass,
	},
	...
	"bypass.schema-invalid": {
		...
		RecoveryHint: "verify schema: hades bypass validate-config (prints the first schema violation); to regenerate, run: hades bypass extract-config (overwrites with a fresh extraction); see docs/operations/bypass.md §schema-reference for the schema documentation",
		Severity:     SeverityError,
		Category:     CategoryBypass,
	},
