...
func (c *Hermes) Run(ctx context.Context) Result {
	...
	if err != nil {
		return Result{
			...
			Details:         "inv-zen-175 requires Hermes Agent >=" + HermesMinVersion + ". The binary `" + HermesBinary + "` was not found in $PATH.",
	...
	if !compareVersionGE(got, HermesMinVersion) {
		return Result{
			...
			Details:         "inv-zen-175 enforces Hermes Agent >=" + HermesMinVersion + ". Found older version.",
