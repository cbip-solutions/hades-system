...
// Package onboard implements Plan 13's shared onboarding infrastructure.
//
...
//
// Per spec §3.4 + inv-zen-031: this package and its subpackages NEVER
...
//   - inv-zen-175 — Hermes Agent ≥0.13.0 (preflight/hermes.go).
//   - inv-zen-176 — plugin format remnants halt (preflight/plugin_format.go).
//   - inv-zen-179 — smart-default confidence ≥0.6 (mcp/smart_default.go).
//   - inv-zen-186 — XDG-canonical path convention (plugin/xdg.go).
//   - inv-zen-189 — cross-platform path tests gate (test infra).
//   - inv-zen-190 — plugin location resolved at install (plugin/location.go).
package onboard
