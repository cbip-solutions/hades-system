...
func NewServer(cfg ServerConfig) (*Server, error) {
	...
	if len(cfg.ReviewerFamilyPool) < cfg.MinPoolSize+1 {
		return nil, fmt.Errorf(
			"audit: ReviewerFamilyPool has %d entries but needs >= %d (MinPoolSize+1=%d) "+
				"to guarantee invariant for any generator family",
	...
	mcp.AddTool(sdk, &mcp.Tool{
		Name:        "audit_review",
		Description: "Review a diff using a cross-provider family-disjoint reviewer. Returns a structured verdict (classification, concerns, suggestions, reviewer_provider, reviewer_model). Generator provider family is excluded from the reviewer pool (invariant).",
		...
		case EmptyPoolHardStop:
			return AuditResponse{}, fmt.Errorf(
				"audit: hard-stop — disjoint reviewer pool empty for generator %q "+
					"(invariant; requires >= %d disjoint families): %w",
	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"generator_provider_family": map[string]any{
				"type":        "string",
				"description": "Provider family that generated the diff (e.g. anthropic, google, deepseek). Excluded from reviewer pool per invariant.",
			},
		},
