...
func NewDoctorSchedulerCmd() *cobra.Command {
  ...
  scheduler.dispatcher.bound      Plan 3 dispatcher reachable (inv-zen-080,
                                  inv-zen-123)
...
func NewDoctorTmuxCmd() *cobra.Command {
  ...
  tmux.server.reachable     /tmp/zen-swarm.sock responsive (inv-zen-117)
  tmux.session.count        active sessions in daemon.db
  tmux.drift.count          orphaned sessions (db says active but tmux disagrees)
  tmux.socket.permissions   /tmp/zen-swarm.sock mode == 0600 (spec §7.3)

Inv-zen-117 anchor: the prober delegates to the live tmux adapter which
