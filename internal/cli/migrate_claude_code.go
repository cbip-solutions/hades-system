...
func newMigrateClaudeCodeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "claude-code",
		Short: "Import a Claude Code installation (~/local agent config/) into Hermes plugin format + hades doctrine",
		Long:  "Imports an existing Claude Code installation (~/local agent config/ + project-local\nlocal agent config/) into HADES's Hermes substrate format. Spec §2.4 mapping table.\n\nModes:\n  --dry-run                  Print plan only; no filesystem changes.\n  --plan-output PATH         Write JSON plan to PATH (deterministic).\n  --apply-plan PATH          Apply a previously generated plan.\n  --preset {strict|lenient}  Strict halts on unmapped surfaces; lenient skips + warns.\n  --force                    Overwrite existing target files.\n  --backup-target            Create tar.gz backup before mutating (invariant).\n  --json                     Emit structured JSON summary.\n\n  (--verify lands in stage when \"hades doctor hermes\" integrates.)\n\nSurfaces imported (spec §2.4 mapping table):\n  ~/local agent config/skills/<name>/SKILL.md           → plugin/hades-system/skills/<name>/SKILL.md\n  ~/local agent config/commands/<name>.md               → plugin/hades-system/commands/<name>.py\n  ~/local agent config/hooks/<event>.{sh,py}            → plugin/hades-system/hooks/<hermes-event>.py\n  ~/local agent config/settings.json#permissions        → doctrines/imported-from-claude-code.toml\n  ~/local agent config/settings.json#model+mcpServers   → config.yaml\n  ~/local agent config/projects/<slug>/memory/*.md      → projects/<slug>/memory/*.md\n  ~/local agent config/.mcp.json#mcpServers             → config.yaml#mcp_servers",
