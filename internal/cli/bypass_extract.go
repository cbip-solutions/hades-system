...
func newBypassExtractCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "extract-config",
		Short: "Extract bypass-config.json v1.0 by capturing upstream client traffic via mitmproxy",
		Long: `Capture an upstream Anthropic client's HTTPS traffic to api.anthropic.com via
mitmproxy, parse the captured flows, and emit bypass-config.json v1.0
(CalVer). Two-step flow: (1) capture launches mitmproxy and prompts the
operator to run the upstream client with HTTPS_PROXY pointing at the listener;
(2) extract parses the captured flows and emits the config. With --capture-only
the second step is skipped (debugging).`,
