...
func newStatePinCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "pin <field> <value>",
		Short: "Set a manual field in system-state.toml (inv-zen-146; emits plan-9-chain event)",
...
--reason is mandatory (inv-zen-146): it records the operator's rationale
in the audit chain event and is visible in ` + "`zen state history`" + `.

A confirmation prompt is shown before the daemon call.
Blank input or anything other than "y"/"yes" aborts without error.`,
		Example: `  zen state pin substrate_min_version 0.7.1 --reason "OpenClaude 0.7.0 has CVE-2026-X"
  zen state pin schema_version 24 --reason "plan-9 schema bump"`,
	...
	c.Flags().String("reason", "", "Operator rationale for the pin (required, inv-zen-146)")
