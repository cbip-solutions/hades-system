...
Required: --project.`,
		Example: `  zen audit-chain verify-chain --project zen-swarm
  zen audit-chain verify-chain --project internal-platform-x --format json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if len(resp.TamperedRecords) > 0 {
				...
				fmt.Fprintf(out, "\nRun: zen audit-chain recover --project %s --from <ts>\n", resp.ProjectID)
			}
			return nil
		},
	}
