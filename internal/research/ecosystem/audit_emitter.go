...
//
// RAGAuditEmitter + RAGAuditChainEmitter interface (Plan 14 Phase A
// Task A-7; master §3.6).
//
// Wraps Plan 9 internal/audit/chain/ primitives indirectly via a
// narrow Plan 14-owned RAGAuditChainEmitter interface (Plan 10 B-6
// pattern). Plan 14 does NOT import internal/daemon/budget; declaring
// the interface HERE lets Plan 14 honour inv-zen-031 boundary while
// still leveraging the same Plan 9 chain hashing/sealing primitives.
//
// Production implementation lands at Phase D Task D-12 — a thin
// wrapper that constructs the chain row + delegates parent_hash /
// self_hash computation to Plan 9 chain primitives. Phase A ships the
...
// logic locally so Phase D wiring is verifiable in isolation.
//
// D-13 (inv-zen-205 full enforcement): added SetProfile rebind hook +
...
type RAGAuditChainEmitter interface {
	...
	// computation atomically (typically mutex-protected; Phase D
	// production wraps Plan 9 chain.Append which uses SQLite txn).
