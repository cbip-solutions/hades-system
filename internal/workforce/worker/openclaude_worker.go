...
// JSON-RPC). It implements the HADES design spec §3.1 Flow 1 pipeline:
//
// 1. Claim a SharedTaskList row (status: pending → in_progress)
// 2. Drain pending FixPromptQueue rows for this worker
// 3. Build a prompt with the doctrine-reinforcement block (invariant hook)
...
// worktreePath. The Worker is the consumer; HADES design WorktreePool will own
...
// identifier for a checkpoint emission. HADES design's reviewer dispatcher
