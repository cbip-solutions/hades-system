...
//
// Boundary discipline (invariant generalized as invariant):
//
...
//
// Atomicity (invariant): the /reload handler does NOT mutate active.Accessor
...
// file-watcher uses. /migrate (invariant) is in-memory ONLY; the daemon
...
//
// Flow (invariant atomic via reload.NotifyForce → sync.Pointer.Store path):
// 1. Decode body.path
