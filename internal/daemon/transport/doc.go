...
// Python Hermes plugin and the Go daemon's Plan 3 dispatcher.
//
// # Architecture
//
// Per spec §1 Q1=B (Plan 11 gitnexus-integration design) AND the post-
...
//     /v1/messages and dispatches via the existing Plan 3 dispatcher chain
...
// # Why three paths
//
// The three paths are NOT redundant — they enforce inv-zen-088 single-egress
// from three angles:
//
...
//     for LLM forwarding. Plan 3's anthropic_proxy.go already hands every
//     request to dispatcher.Forward; this package wraps that with a
//     transport-aware shim that emits a Plan 9 Tessera anchor when the call
...
//     SDK against the daemon socket. inv-zen-164 compliance scans verify
//     no second LLM path exists in plugin Python source.
//
// # Compile-time invariant (inv-zen-164)
//
...
//
// This package imports providers (Plan 3) + redact (Plan 2 secret marker)
...
//   - mcpgateway (Phase A) owns: Server, Aggregator, ToolRegistry, RBAC,
//     CaronteProxy.
//   - augment (Phase C) owns: Pipeline, DoctrineGate, BudgetGate, etc.
//   - citation (Phase D) owns: Envelope, Renderer, MarkdownFallback.
//
...
// graceful-degradation paths (e.g. audit anchors at nil → no Plan 9
