...
// consumers (anthropic_proxy handler in, MCPs, workforce
...
// is what makes the architecture testable end-to-end with in-memory
// fakes.
//
...
// supplied Forwarder is. The dispatcher documents this guarantee at
// its package level.
//
// Phase scope (B-6): profile resolution + dispatcher invocation.
// adds a budget pre-check that consults the budget MCP `cap_status`
...
// fans out one Forward per inbound HTTP request). The dispatcher
