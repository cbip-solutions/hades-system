...
//     once at routine creation time (Phase D httptrigger.go) and never
...
// comparison (inv-zen-132). The package never imports internal/store
// (inv-zen-031); the per-routine token store is reached via the
// HTTPTokenStore interface defined locally and satisfied by Phase D's
...
//
// inv-zen-131: every /v1/* route (except the two bearer endpoints) MUST
// be wrapped by PeerCredOnly. Compliance test enforces.
//
// inv-zen-132: per-routine bearer MUST use ConstantTimeCompare AND emit
