...
// Package auth implements the hades-ctld HTTP auth boundary
// .
//
// Three authentication classes:
//
// 1. Unix-socket peer-cred (default for /v1/*; PeerCredOnly middleware).
// Caller must connect via UDS /tmp/hades-system.sock (mode 0700) OR via
...
// restart) and persisted to ~/.config/hades-system/daemon-bearer.txt
