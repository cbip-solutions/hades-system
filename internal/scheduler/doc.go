...
//
// File layout (one task per file per Phase D plan):
//
//	scheduler.go         — D-2 enums + value types + sentinel errors
//	store_iface.go       — D-2 Store interface (inv-zen-031 bridge)
//	quota_iface.go       — D-2 QuotaPreFlightChecker (Phase B contract)
//	dispatcher_iface.go  — D-2 Dispatcher (inv-zen-080 single-egress)
//	eventlog_iface.go    — D-2 EventEmitter (Phase F contract)
...
//	jitter.go            — D-4 deterministic jitter (inv-zen-120)
...
//	loop.go              — D-9 session-bound polling (Phase C lifecycle)
...
//
//   - inv-zen-031 — package never imports internal/store; bridge via
//     internal/daemon/scheduleradapter satisfying Store interface.
//   - inv-zen-080 / inv-zen-123 — package never imports
//     internal/providers or private-tier1-module; LLM dispatch
//     goes only through the Dispatcher interface.
//   - inv-zen-120 — Scheduler jitter offset deterministic
//     (jitterDeterministicSentinel anchor in sentinel.go).
//   - inv-zen-121 — Per-doctrine miss policy correctly applied
