...
func Render(doc BriefDoc) string {
	if len(doc.Items) > MaxBriefItems {
		panic(fmt.Sprintf(
			"zenday.Render: inv-zen-126 violation: len(items)=%d > MaxBriefItems=%d",
			len(doc.Items), MaxBriefItems))
	}
	if !IsSorted(doc.Items) {
		panic("zenday.Render: inv-zen-127 violation: IsSorted(items) = false")
	}
	...
	default:
		panic(fmt.Sprintf("zenday.Render: unknown BriefType %d", doc.Type))
	}
...
const morningTemplate = "# zen day — {{.Date.Format \"2006-01-02\"}} morning brief\n" +
	"{{$gates := .GroupGates}}" +
	"{{$costs := .GroupCosts}}" +
	...
	"+ {{.TruncatedCount}} more in `zen inbox --since 24h`\n" +
	"{{end}}"

const eodTemplate = "# zen day — {{.Date.Format \"2006-01-02\"}} EOD digest\n" +
	"\n" +
...
func mustExecute(tmpl *template.Template, data any) string {
	...
	if err := tmpl.Execute(&buf, data); err != nil {
		panic(fmt.Sprintf("zenday.Render: %s template exec: %v", tmpl.Name(), err))
	}
	return buf.String()
}
...
func renderNotifications(s *NotificationsSection) string {
	...
	if s.PendingAcks > 0 {
		b.WriteString(" (zen inbox --since 24h)")
	}
	b.WriteString("\n")
	...
	if s.HermesDispatchErrors > 0 {
		b.WriteString(" (investigate: zen doctor hermes)")
	}
	b.WriteString("\n")
	return b.String()
}
