...
func auditProjectAction(worst auditSubstrateStatus, alias string) string {
	switch worst {
	case auditStatusFail:
		return fmt.Sprintf("zen audit status --project %s", alias)
	case auditStatusWarn:
		return fmt.Sprintf("zen audit history --project %s --since 7d", alias)
	default:
		return ""
	}
}
...
func init() {
	...
	if RankCritical != RankOperatorGate {
		panic("zenday audit_section: RankCritical alias drift from RankOperatorGate")
	}
	if RankAlertNeeded != RankUrgentEvent {
		panic("zenday audit_section: RankAlertNeeded alias drift from RankUrgentEvent")
	}
	if RankInfoSummary != RankInfoImmediate {
		panic("zenday audit_section: RankInfoSummary alias drift from RankInfoImmediate")
	}
}
