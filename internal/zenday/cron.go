...
// Package zenday — `RegisterDefaultRoutines` cron registration helper +
// `~/.config/zen-swarm/zen-day.toml` operator override.
//
...
// mutations). When the operator edits zen-day.toml between boots and
...
const (
	...
	MorningRoutineID = "zenday-morning"

	EODRoutineID = "zenday-eod"
...
var configRelPath = filepath.Join(".config", "zen-swarm", "zen-day.toml")

func LoadZenDayConfig(homeDir string) (ZenDayConfig, error) {
	cfg := ZenDayConfig{
	...
	if err != nil {
		...
		return cfg, fmt.Errorf("zenday: read %q: %w", path, err)
	}
	if _, err := toml.Decode(string(body), &cfg); err != nil {
		return cfg, fmt.Errorf("zenday: parse %q: %w", path, err)
	}
...
func RegisterDefaultRoutines(ctx context.Context, store scheduler.Store, homeDir string, now time.Time) error {
	cfg, err := LoadZenDayConfig(homeDir)
	if err != nil {
		return err
	}
	...
	for _, e := range entries {
		...
		if err := upsertRoutine(ctx, store, e.id, e.action, e.cfg, now); err != nil {
			return fmt.Errorf("zenday: %s routine: %w", e.id, err)
		}
	}
	return nil
}
