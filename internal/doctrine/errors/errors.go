...
var (
	...
	ErrTightenViolation = errors.New("doctrine: per-project override attempts to loosen baseline (inv-zen-136)")
	...
	ErrTransverseOverrideAttempted = errors.New("doctrine: transverse axiom override attempt rejected (inv-zen-135 hardcoded operator-only)")
)
...
func (e *TransverseOverrideAttempt) Error() string {
	if len(e.Fields) > 0 {
		return fmt.Sprintf("doctrine: source=%s section=%s attempted to override transverse fields [%s] (inv-zen-135 hardcoded operator-only)",
			e.Source, e.Section, strings.Join(e.Fields, ","))
	}
	return fmt.Sprintf("doctrine: source=%s section=%s attempted to override transverse axioms (inv-zen-135 hardcoded operator-only)",
		e.Source, e.Section)
}
