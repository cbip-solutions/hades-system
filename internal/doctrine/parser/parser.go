...
// v1 Schema struct (Plan 8 Q2 A: no merge engine; one TOML = one
// effective doctrine). The parser is the SOLE entry point used by the
// embedded built-in loader (Phase D, //go:embed) and the user-override
// reload loop (Phase G, fsnotify) so both surfaces share one code path
...
// *errors.TransverseOverrideAttempt (inv-zen-135 enforcement).
//
...
//
// Boundary parser ⊥ internal/store (inv-zen-133). No store imports;
...
//   - Embedded built-in TOMLs (Phase D, //go:embed): ParseOpts{AllowTransverseDeclaration: true}
//   - User override / per-project override (Phase G, fsnotify): ParseOpts{} (default)
//
// Passing the wrong opts is an inv-zen-135 vulnerability; integration
// tests in Phase D/G assert the correct opts at each call site.
type ParseOpts struct {
	...
	// *errors.TransverseOverrideAttempt when they do (inv-zen-135).
...
// "project:/repo/.zen/doctrine-override.toml", etc.).
//
// Pre/postconditions:
...
//     fill any field absent from the input per Phase A's struct tags.
...
//
// Example (Phase D built-in load path):
//
