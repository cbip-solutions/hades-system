...
// Package hadesday — EOD digest idempotent composer.
//
...
// command); hadesday consumes them verbatim into the rendered markdown.
package hadesday
...
// ProjectConfigReader is the contract hadesday consumes for per-project
// opt-out resolution. Production wire-up satisfies via
// projectctx-derived loaders that read [project].hades_day_eod_summary
// from each project's hadessystem.toml; tests substitute fakes.
//
...
func emitEODReady(ctx context.Context, emitter EventEmitter, payload EODDigestReadyPayload) error {
	if emitter == nil {
		return fmt.Errorf("hadesday: nil EventEmitter")
	}
