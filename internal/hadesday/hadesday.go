...
// HTTP handlers (`internal/daemon/handlers/hadesday.go`) via
// `type HadesDayItem = hadesday.BriefItem`. review CRITICAL #9
// reconciliation (2026-05-01): MUST NOT declare a parallel
// `HadesDayItem` struct that drops fields silently.
...
func (bi BriefItem) Validate() error {
	if !bi.Rank.Valid() {
		return fmt.Errorf("hadesday.BriefItem: rank invalid (got %d, want 1..7)", bi.Rank)
	}
	if bi.Message == "" {
		return errors.New("hadesday.BriefItem: message required")
	}
	return nil
}
...
var (
	ErrAlreadyGenerated = errors.New("hadesday: today's brief already generated (use --force to overwrite)")

	ErrSourceCollectFailed = errors.New("hadesday: every source collection leg failed")

	ErrCollectCancelled = errors.New("hadesday: collect cancelled")
)
