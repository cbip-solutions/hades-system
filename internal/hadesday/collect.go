...
// Package hadesday — parallel source fan-out collector.
//
...
func collectSchedulerLeg(ctx context.Context, store SchedulerHistorian, since, now time.Time) legResult {
	...
	for _, h := range hist {
		...
		out = append(out, BriefItem{
			...
			Action:    fmt.Sprintf("hades schedule run --now %s", h.ScheduleID),
	...
	for _, s := range snaps {
		...
		case "paused":
			out = append(out, BriefItem{
				...
				Action:    fmt.Sprintf("hades autonomy ack %s", s.ProjectAlias),
