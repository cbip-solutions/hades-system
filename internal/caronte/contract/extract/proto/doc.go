...
// Package proto is the Plan 20 gRPC + .proto IDL extractor namespace
// (Language: LangProto). The concrete RouteExtractor implementation lands in
// Phase D (master row D, wave W3) as proto.go alongside fixtures under
// fixtures/. The (LangProto, "grpc") tuple is reserved for this package by
// Phase D's daemon-time Register() call; until then this package holds the
...
//     research catalog.
//
// Boundary this package and Phase D's code under it MUST NOT import
// internal/store (inv-zen-230 + Plan 20 inv-zen-271); the only store access is
// via internal/caronte/store APIEndpoint / APICall types Phase B provides.
package proto
