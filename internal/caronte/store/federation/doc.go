...
// breaking_change_consumers) and the CRUD surface Plan 20's later phases
// (linker, breaking-change detector, L10 coordinator, MCP surface, TUI)
// write through.
//
// Storage location (Plan 20 §4 D6): the workspace.db lives at the daemon
...
// project-scoped — inv-zen-163 separation: per-project caronte.db lives at
// <canonical>/.zen/caronte.db, this is the orthogonal cross-project
// federation tier).
//
// Boundary (inv-zen-271): this package and ALL its callers under
...
// ContractLink + WorkspacePolicy + the sentinel error set Plan 19 M ships).
// The compliance test
...
//
// Audit (inv-zen-269, ADR-0116 / D10): every workspace-level write
// (contract_links INSERT, breaking_changes INSERT, coordinated-fix dispatch,
// federated-query denied-access, workspace-policy mutation) emits a Plan 14
...
// is the only Plan-20 AppendLeaf call site. The FIX-5 AuditEmitter
// interface + NewAuditEmitter(adapter, workspaceID) constructor provide
// the per-workspace adapter Phase F + Phase J wire into the consumers.
//
// Capa-firewall extension (inv-zen-264): every persistent contract_links /
// breaking_changes write transits store.Workspace.authorize() (Plan 19 M's
...
// before — so the capa-firewall contract from Plan 19 M extends to Plan 20
...
//
// inv-zen-129: this package makes NO web calls; all SQLite + Tessera I/O is
// local.
package federation
