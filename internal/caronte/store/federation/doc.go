...
// breaking_change_consumers) and the CRUD surface HADES design's later phases
// (linker, breaking-change detector, L10 coordinator, MCP surface, TUI)
// write through.
//
// Storage location: the workspace.db lives at the daemon
// state path — $HADES_STATE_DIR/hades-system/workspace.db if set, else
// $XDG_STATE_HOME/hades-system/workspace.db (Linux), else
...
// ContractLink + WorkspacePolicy + the sentinel error set HADES design M ships).
// The compliance test
...
// federated-query denied-access, workspace-policy mutation) emits a HADES design
...
// tests/compliance/inv_hades_269_audit_every_write_test.go asserts EmitAudit
// is the only HADES design AppendLeaf call site. The FIX-5 AuditEmitter
...
// breaking_changes write transits store.Workspace.authorize() (HADES design M's
// chokepoint at workspace.go:117). The seam swap in
// internal/caronte/store/workspace.go (task) routes through this
// package's LinkStore port AFTER the existing authorize() gate — never
// before — so the capa-firewall contract M extends to HADES design
