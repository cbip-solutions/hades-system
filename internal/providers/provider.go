...
// 's bypass.Tier (so audit rows written before HADES design remain
// readable when re-decoded), and HADES design appends new tiers after.
//
...
const (
	...
	// formerly reported this tier was removed in HADES design — the
	// providers.toml-driven cascade superseded it (ADR-0093). This
	// constant is KEPT, not deleted: its String() value "openclaude"
	// is persisted in cost_ledger.tier for pre-HADES design historical rows,
...
// TierBackend is the contract every HADES design backend satisfies.
//
