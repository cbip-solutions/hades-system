...
// Decision Record) machine-readable index per Plan 9 spec §1 Q7 A.
//
// The package owns the ADR frontmatter contract (defined in
// docs/decisions/_schema.json — JSON Schema Draft-07), parses YAML
...
// edges) consumed by `make verify-invariants` per inv-zen-151,
...
// # Invariants
//
//   - inv-zen-147: ADR id is unique repo-wide. The validator enforces
...
//
//   - inv-zen-151: dual JSON manifest (_index.json + _graph.json)
...
//
//   - inv-zen-031: this package never imports internal/store; chain
//     integration via internal/daemon/auditadapter/ (Phase B).
//
// # State machine
//
...
// Phase E's migrate.go parses these via regex, emits Structured
