...
// coordinator (Plan 5 Phase H). The coordinator drives the
...
// # Layers
//
// Phase H exposes three reviewer layers as Layer values:
//
...
// manual-only per Pulido §3.5 claim-strength tier — Plan 8 wires the
...
// Phase A eventlog. Each Subscription is filtered by ProjectID
...
//
// # Boundary discipline (inv-zen-089, inv-zen-090)
//
// Phase H imports only internal/orchestrator/clock and
// internal/orchestrator/eventlog. It never imports
// internal/orchestrator (Phase D Orchestrator) or internal/store —
...
// time-accelerated test tier (Phase O) drives cadence deterministically;
// direct calls to time.Now / time.NewTicker are forbidden inside this
// package and a vet-style lint (Phase O) enforces.
//
// # References
//
