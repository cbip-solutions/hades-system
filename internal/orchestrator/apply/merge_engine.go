...
// MergeOutcome is what HADES design's real engine returns. HADES design callers use
...
// after the merge.
// - ReviewerSummary — free-form rationale for audit consumers + HADES design
// hash-chain replay.
...
// MergeEngine is the cross-worker integration contract. HADES design ships ONLY
// this interface declaration; HADES design implements it. Callers MUST tolerate
// a fake-only mode in HADES design by skipping cross-worker scenarios in tests
// ).
//
// design choice D rationale: live correction is single-worker-branch sequential and
...
// is by concern, not by stage ordering — each engine has a distinct
...
func (errMergeNotImpl) Error() string {
	return "apply.MergeEngine: HADES design not yet shipped — fake-only mode"
}
