...
// lifecycle and HADES design's workforce.Manager. Given a DispatchRequest it:
//
...
// only worker-side string surfaced is r.Err.Error() (HADES design's worker
...
// The HADES design workforce.Manager is consumed via the WorkforceManager
// interface declared here so eventlog (durable) ⊥ queue (transient)
// stays a clean separation; bootstrap ( adapter / daemon
// main) wires the real impl when HADES design is merged.
...
// SpawnRequest is the HADES design contract input. ships the minimum
// fields HADES design's Manager.Spawn signature requires; richer fields
// (HRA reviewer hierarchy, cost-tier fan-out, doctrine-driven retry
// budget) land in HADES design Phases F/G/H/M without narrowing this struct.
//
...
// the lease lifecycle: HADES design workers MUST NOT call Release.
