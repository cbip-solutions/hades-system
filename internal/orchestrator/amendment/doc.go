...
// (inv-zen-103: Plan 5 ADR slots 0020-0029).
//
// # Architecture
//
// The package is event-driven and operator-gated. AmendmentProposer
// subscribes to the Phase A event log via the narrow EventEmitter
...
// docs/decisions/proposed/NNNN-*.md and a DoctrineAmendmentProposed
// event is emitted. Operator acknowledgement (via Phase F's
// OperatorGate) drives AmendmentApplier.Apply: validate-first
// (inv-zen-098), capture pre-apply TOML SHA-256, atomic git commit,
...
// (inv-zen-094). Operator deny moves the ADR to
// docs/decisions/rejected/ and arms a per-doctrine cooldown timer
// suppressing re-proposal of the same trigger pattern.
//
// # Boundaries (inv-zen-089 generalized)
//
//   - amendment ⊥ internal/store — no SQLite import. Bridged in
//     daemon/orchestratoradapter (Phase N).
...
//     Phase H wires the concrete L4 reviewer.
...
//
//   - inv-zen-094 — Amendment rollback atomicity. AmendmentApplier.Apply
...
//   - inv-zen-098 — Doctrine validate before apply. Apply MUST call
...
//   - inv-zen-103 — ADR range reservation. Plan 5 reserves
