...
// Package aggregator implements the Q13 C per-rule
// SIGNAL with shared sliding-window machinery + 3 generic event-
// categories (cost / merge / recovery).
//
// Each aggregator subscribes to a bounded set of eventlog
...
//
// - ships `eventlog.PayloadEncoder` (typed-event interface) +
...
//
// - events live in internal/orchestrator/merge as a separate
// package-local enum. avoids cross-importing merge directly
// by accepting any PayloadEncoder
// implementation. Bridge code at the N adapter layer translates
