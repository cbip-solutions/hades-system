...
"""HadesSystemTransport — Python class that forwards LLM requests via HTTP"""
...
class CompletedResponse:
    """Result of a ``HadesSystemTransport.complete`` (or single-chunk
    ``stream``) call.
...
HEADER_TRANSPORT_SOURCE = "X-HADES-Transport"
TRANSPORT_LABEL = "hadessystem"
...
# Kwargs that HadesSystemTransport.complete handles itself (envelope metadata);
...
class HadesSystemTransport:
    """LLM-dispatch transport that routes via hades-ctld.
    ...
    NOTE: this class is the substrate for DIRECT callers (hades CLI Python
    wrappers, MCPs, integration tests). Hermes itself does NOT drive this
    class — Hermes uses the ``ProviderProfile`` registered in
    ``plugin/hades-system/providers/__init__.py`` to point its native
    ...
    def __init__(self, socket_path: str | None = None) -> None:
        ...
        Args:
            socket_path: Optional explicit override for the daemon Unix-
                socket path. Falls back to ``$HADES_DAEMON_SOCKET`` or
                ``/tmp/hades-system.sock``. Explicit argument wins over env.
        """
        self._socket_path = socket_path or os.environ.get(
            "HADES_DAEMON_SOCKET", "/tmp/hades-system.sock"
        )
    ...
    def complete(
        ...
        **kwargs: Any,
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"hades-system daemon returned status {response.status_code}: "
                f"{response.text!r}"
            )
        try:
